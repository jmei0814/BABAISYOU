...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//reg += 2;
	u8 sendbuf[] = {(u8)(reg+2),(u8)(val)};

	if (XSpi_Transfer(SpiInstancePtr,sendbuf,NULL, 2) != 0) { xil_printf("error_reg_wr");}
//
//	u8 sendbuf = reg+2;
//	if (XSpi_Transfer(SpiInstancePtr,&sendbuf,NULL, 1) != 0) { xil_printf("error_reg_wr");}
//	sendbuf = val;
//	if (XSpi_Transfer(SpiInstancePtr,&sendbuf,NULL, 1) != 0) { xil_printf("error_reg_wr");}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if (XSpi_Transfer(SpiInstancePtr,sendbuf,NULL, nbytes+1) != 0) { xil_printf("error_byte_wr");}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	u8 sendbuf[2] = {(u8)(reg),0};
	u8 returnbuf[2] = {0,0};

	if (XSpi_Transfer(SpiInstancePtr,sendbuf,returnbuf, 2) != 0) { xil_printf("error_rd");}

	XSpi_SetSlaveSelect(SpiInstancePtr, 0);

	return (BYTE)(returnbuf[1]);
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 recevbuff[nbytes+1];


	if (XSpi_Transfer(SpiInstancePtr,sendbuf,recevbuff, nbytes+1) != 0) { xil_printf("error_byte_rd");}

	XSpi_SetSlaveSelect(SpiInstancePtr, 0);

	for (int i = 1; i <= nbytes ; i++){
		data[i-1] = recevbuff[i];
	}
	return data + nbytes;
}
